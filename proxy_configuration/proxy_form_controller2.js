 ...
 */
 ...
 if(!obj)return null;
var it,key,l,currobj={obj:obj,keys:Object.keys(obj),index:0},nest=[];
mai:while(currobj){
...
currobj={obj:it,keys:Object.keys(it),index:0};
continue mai;
}else{
...
var ProxyFormController = function(id) {
  ...
  this.proxySelect=document.getElementById('proxySchemeForAll');
...
ProxyFormController.specificProxies=["singleProxy","proxyForHttp","proxyForHttps","proxyForFtp","fallbackProxy"];
...
ProxyFormController.button= ["edit","delete"];
 ...
 * @static
 */
ProxyFormController.getPersistedSettings = function(varname) {
  var result = null;
  if (window.localStorage[varname] !== undefined)
    result = JSON.parse(window.localStorage[varname]);
 ...
 * @static
 */
ProxyFormController.setPersistedSettings = function(varname,config) {
  window.localStorage[varname] = JSON.stringify(config);
};
...
///////////////////////////////////////////////////////////////////////////////
  ...
  if(this.isIncognitoMode_()!=a[0].incognito){console.log("hue");this.toggleIncognitoMode_()}}.bind(this))
   ...
   * @private
   */
  handleState_: function(c,incognito) {
    if (c.levelOfControl === ProxyFormController.LevelOfControl.AVAILABLE ||
	  ...
	  this.config={defaultconf:{proxyType:"singleProxy"},profileList:[{name:"Direct",blocked:true,conf:{mode:ProxyFormController.ProxyTypes.DIRECT}},{name:"System",blocked:true,conf:{mode:ProxyFormController.ProxyTypes.SYSTEM}}]};
	  var settings=ProxyFormController.getPersistedSettings("profileList");
	  if(settings)
	  this.config.profileList=this.config.profileList.concat(settings);
      if(incognito){
 ...
 handleRegularState_: function(c) {
  ...
  bindEventHandlers_: function() {
   ...
   proxyGenerate:function(str){
    var proxyConf={mode:ProxyFormController.ProxyTypes.FIXED},rules={},prox=str.split(':');
	if(!str.trim())return null;
	rules.singleProxy={host:prox[0],port:+prox[1]};
	if(this.proxySelect.value!=="same"){
	rules.singleProxy.scheme=this.proxySelect.value;
	}
	proxyConf.rules=rules;
	return proxyConf;
  },
  ...
  dispatchFormClick_: function(e) {
    ...
    //if (t.nodeName === 'INPUT' && t.getAttribute('type') === 'submit') {
	 if (t.classList.contains("proxyb")) {
      return this.applyChanges_(e);

    
    } else if(t.id=="save"){
	e.preventDefault();
	var proxies=document.getElementById('proxies').value.split('\n');
for(var iter=0,len=proxies.length,item;iter<len;iter++){
	item=proxies[iter];
	var genproxy=this.proxyGenerate(item);
	if(genproxy)
	this.config.profileList.push({conf:genproxy});
	}
	ProxyFormController.setPersistedSettings('profileList',this.config.profileList.slice(2));
	this.readCurrentState_();
	}else if(t.dataset.el=="proxy"){
	switch(t.dataset.type){
	case "delete":
	this.config.profileList.splice(+t.parentNode.parentNode.dataset.numb,1);
	break;
	case edit:
	break;
	}
...
// Case 2: "Use the same proxy for all protocols" in an active section
  ...
  applyChanges_: function(e) {
    ...
    if (this.isIncognitoMode_())
	  ...
	  this.config_.incognito = this.config.profileList[e.target.dataset.numb].conf;
    else
      //this.config_.regular = this.generateProxyConfig_();
	  this.config_.regular = this.config.profileList[e.target.dataset.numb].conf;

    chrome.proxy.settings.set(
        {value: this.config_.regular, scope: 'regular'},
        this.callbackForRegularSettings_.bind(this));
    chrome.extension.sendMessage({type: 'clearError'});
  },
  ...
  callbackForRegularSettings_: function() {
    ...
    if (this.config_.incognito) {
      ...
      ProxyFormController.setPersistedSettings('proxyConfig',this.config_);
...
*/
  ...
  recalcFormValues_: function(c) {
      var prlist=document.getElementById("proxylist");
	  prlist.innerHTML="";
	  
	  this.config.profileList.forEach(function(a,i){
	  var div,li=document.createElement("li");
	  li.appendChild(div=document.createElement("div"));
	  if(!a.name){
	  for(var iter=0,len=ProxyFormController.specificProxies.length,item;iter<len;iter++){
	  item=ProxyFormController.specificProxies[iter];
	  if(a.conf.rules[item].host){
	  a.name=a.conf.rules[item].host+(a.conf.rules[item].port?":"+a.conf.rules[item].port:"");
	  break;
	  }
	  }
	  }
	  div.appendChild(document.createTextNode(a.name?a.name:"No name defined"));
	  div.dataset.numb=i;
	  div.classList.add("proxyb");
	  if(!a.blocked){
	  //Make edit buttons and stuff
	  var ul=document.createElement("ul");
	  for(var iter=0,len=ProxyFormController.button.length;iter<len;iter++){
	  var button=document.createElement('button'),li2;
	  button.classList.add("linkbutton");
	  button.dataset.el="proxy";
	  button.dataset.type=ProxyFormController.button[iter]
	  button.appendChild(document.createTextNode(ProxyFormController.button[iter]));
	  (li2=document.createElement('li')).appendChild(button);
	  ul.appendChild(li2);
	  }
	  div.appendChild(ul);
	  }
	  ...
	  var div=document.getElementById('proxsett'),str=[];
	  objectLoop(c,function(a,b,i,o){
	  var white="";
	  while(i--)white+=" ";
	  str.push(white+b+" : "+(o?"->":JSON.stringify(a)));
	  })
	  div.innerHTML=("Current proxy settings are:<br><pre class='proxycon'>"+str.join('\n')+"</pre");
	  this.form_.appendChild(div);
